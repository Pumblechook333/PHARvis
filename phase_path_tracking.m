...
for nhops = 1:1:nhop_max
    ...
    phasepath_arr = zeros(rsto,num_elevs);
    for i = rsta:rinc:rsto
        ...
        phasepaths = zeros(1,num_elevs);
        % Find maximum height of rays
        
        for elev = 1:1:num_elevs
            grounded_ray = ray_data_N(elev).ray_label(end) == 1;
            if grounded_ray
                in_range = IONS.chk_dist(range, ray_N(elev), wgs84) == 1;
                if in_range
%                     ph_pth = ray_data_N(elev).phase_path(1);
                    ph_pth = ray_data_N(elev).phase_path(end);
                    
                    phasepaths(elev) = ph_pth;
                end
            end
        end

        phasepath_arr(i,:) = phasepaths;

    end
    phase_paths.(hop_field) = phasepath_arr;
...
for nhops = 1:1:nhop_max
    ...
    for k = rsta:rinc:rsto
        ...
        for i = 1:1:num_elevs
            y = phase_paths.(hop_field)(k,i) / (1e3);
...
ti = "Phase Path of Rays Across Day";
title(ti, 'FontSize', fs)
xlabel('Time (UT)', 'FontSize', fs)
ylabel('Phase Path (km)', 'FontSize', fs)
